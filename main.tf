...
resource "null_resource" "WAIT_INSTANCE_STATUS" {
    count = (length(var.INSs) > 0 ?
            length(var.INSs) : 0)
    ...
    provisioner "local-exec" {
        command = <<-EOF
        EC2_INS_STATUS=$(aws ec2 describe-instance-status --instance-ids ${aws_instance.INS[count.index].id} --query 'InstanceStatuses[].InstanceStatus[].Status' --output text --profile ${var.PROFILE})
        EC2_SYS_STATUS=$(aws ec2 describe-instance-status --instance-ids ${aws_instance.INS[count.index].id} --query 'InstanceStatuses[].SystemStatus[].Status' --output text --profile ${var.PROFILE})
        while [[ $EC2_INS_STATUS != "ok" ]] && [[ $EC2_SYS_STATUS != "ok" ]]; do
            sleep 10
            EC2_INS_STATUS=$(aws ec2 describe-instance-status --instance-ids ${aws_instance.INS[count.index].id} --query 'InstanceStatuses[].InstanceStatus[].Status' --output text --profile ${var.PROFILE})
            EC2_SYS_STATUS=$(aws ec2 describe-instance-status --instance-ids ${aws_instance.INS[count.index].id} --query 'InstanceStatuses[].SystemStatus[].Status' --output text --profile ${var.PROFILE})                  
        done
...
# If use below resource, Terraform tfstate will not recognized existing NIC, so NIC will be crashed whenever reapply
